...
// لاحظ هنا أننا نستقبل property و agent من لوحة التحكم
export default function PropertyCardExport({ property, agent, onClose }) {
    const cardRef = useRef(null);
    const [isExporting, setIsExporting] = useState(false);
    const [exportedImage, setExportedImage] = useState(null);

    // ربط البيانات الحقيقية القادمة من قاعدة البيانات، مع الاحتفاظ ببيانات بديلة في حال كان الحقل فارغاً
    const data = {
        title: property?.title || 'فيلا فاخرة للإيجار',
        location: [property?.city, property?.district].filter(Boolean).join(' - ') || property?.location || property?.address || 'الرياض - حي النرجس',
        refId: property?.id ? String(property.id).substring(0, 8).toUpperCase() : 'A-2024-1258',
        price: property?.price ? Number(property.price).toLocaleString() : '120,000',
        area: property?.area || property?.space || '450',
        age: property?.age || property?.property_age || 'جديد',
        rooms: property?.bedrooms || property?.rooms || '5',
        baths: property?.bathrooms || property?.baths || '6',
        halls: property?.living_rooms || property?.halls || '2',
        kitchens: property?.kitchens || '1',
        type: property?.type || property?.property_type || 'فيلا',
        ...
        phone: agent?.phone || '050xxxxxxx',
        social: agent?.username || 'mthaal.realestate',
        agencyName: agent?.office_name || 'مكتبي العقاري',
        agencyLogo: agent?.office_logo_url || agent?.profile_image_url || null,
        // جلب الصور الحقيقية للعقار (أول 3 صور)
    ...
    const handleExport = async () => {
        ...
        try {
            // تحميل مكتبة html2canvas بشكل ديناميكي آمن
                ...
                >
                    ...
                    <div className="flex justify-between items-center p-6 border-b border-gray-100">
                        ...
                        <div className="flex flex-col items-center max-w-[120px]">
                            <div className="w-12 h-12 border-2 border-gray-800 rounded-lg flex items-center justify-center mb-1 overflow-hidden bg-white">
                                {data.agencyLogo ? (
                            ...
                            <span className="text-sm font-bold text-gray-800 text-center leading-tight truncate w-full">{data.agencyName}</span>
                            <span className="text-[10px] text-gray-500 mt-0.5">خدمات عقارية متكاملة</span>
                        </div>
                        ...
                        {/* Platform Logo */}
                        <div className="flex flex-col items-center gap-1 text-red-500 font-bold text-lg max-w-[120px]">
                            <span className="bg-red-100 text-red-600 px-3 py-1 rounded-full text-xs whitespace-nowrap">عقار كلاود</span>
                        </div>
                    </div>
                    ...
                    <div className="relative p-6">
                        ...
                        <div className="grid grid-cols-3 gap-4 h-[400px]">
                            {/* Main Large Image */}
                            <div className="col-span-2 row-span-2 rounded-xl overflow-hidden shadow-sm relative bg-gray-100">
                                <img src={data.mainImage} alt="Main Property" crossOrigin="anonymous" className="w-full h-full object-cover" />
                            </div>
                            {/* Side Images */}
                            <div className="rounded-xl overflow-hidden shadow-sm relative bg-gray-100">
                        ...
                        <div className="col-span-1 bg-gray-50 rounded-xl p-4 border border-gray-100">
                            ...
                            <table className="w-full text-sm text-right">
                                <tbody>
                                    ...
                                    <tr className="border-b border-gray-200">
                                        <td className="py-1 text-gray-500 whitespace-nowrap">النوع</td>
                                        <td className="py-1 font-semibold text-gray-800 text-left">{data.status}</td>
                                    </tr>
                                    <tr>
                                        <td className="py-1 text-gray-500 whitespace-nowrap">رقم الترخيص</td>
