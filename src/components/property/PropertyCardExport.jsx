...
export default function PropertyCardExport({ property, agent, onClose }) {
    ...
    // تجهيز البيانات
    const data = {
        title: property?.title || 'عنوان العقار غير متوفر',
        location: [property?.city, property?.district].filter(Boolean).join(' - ') || property?.location || property?.address || 'الموقع غير متوفر',
        // أخذ جزء من ID العقار ليكون رقم عرض، وإذا لم يوجد يكتب غير متوفر
        refId: property?.id ? String(property.id).substring(0, 8).toUpperCase() : 'غير متوفر',
        price: property?.price ? Number(property.price).toLocaleString() : '0',
        area: property?.area || property?.space || '-',
        age: property?.age || property?.property_age || 'جديد',
        ...
        // جلب المميزات الديناميكية من قاعدة البيانات
        features: property?.features || property?.amenities || []
    };

    // دالة لاختيار أيقونة مناسبة للميزة بناءً على النص، أو وضع أيقونة افتراضية
    ...
    const handleExport = async () => {
        ...
        try {
            const canvas = await html2canvas(cardRef.current, {
                ...
                logging: false, // تعطيل اللوجز لتسريع العملية
                onclone: (clonedDoc) => {
        ...
        <div className="flex flex-col items-center w-full max-w-4xl mx-auto font-[Cairo]">
            ...
            <div className="bg-gray-200 p-8 rounded-xl w-full flex justify-center overflow-x-auto">
                ...
                >
                    ...
                    <div className="flex justify-between items-center p-6 border-b border-gray-100">
                        {/* Agency Logo */}
                        {/* تم إزالة overflow-hidden لتجنب قص النصوص السفلية وإضافة pb-2 */}
                        <div className="flex flex-col items-center max-w-[160px] min-w-[120px]">
                            ...
                            {/* تم إزالة leading-tight و truncate وإضافة مساحة سفلية pb-1 */}
                        ...
                        <div className="text-center flex-grow px-4">
                            {/* تم إزالة leading-tight وإضافة pb-2 لتفادي تكسر الخطوط */}
                        ...
                        {/* تم إزالة منصة عقار كلاود للحفاظ على المساحة بناء على طلبك */}
                    ...
                    <div className="relative p-6">
                        ...
                        <div className="absolute bottom-10 right-10 z-10 bg-white/95 backdrop-blur rounded-xl p-4 shadow-lg border border-gray-100 text-center w-44">
                            ...
                            <div className="text-sm text-gray-500 font-medium">ريال سعودي</div>
                        </div>
                    ...
                    {/* Features Section (Dynamic from Database) */}
                    {data.features && data.features.length > 0 && (
                        <div className="px-8 mt-8">
                            ...
                            <div className="flex justify-around text-center px-10 flex-wrap gap-y-4">
                                {/* نأخذ أول 4 مميزات فقط حتى لا يتشوه التصميم */}
                                {data.features.slice(0, 4).map((feature, index) => (
                                    <div key={index} className="flex flex-col items-center min-w-[100px]">
                                        {getFeatureIcon(feature?.name || feature)}
                                        <span className="text-sm text-gray-600 font-medium">{feature?.name || feature}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="col-span-1 bg-gray-50 rounded-xl p-4 border border-gray-100 flex flex-col justify-center">
                            ...
                            <div className="space-y-4 text-sm">
                                <div className="flex items-center gap-3" dir="ltr">
                                    ...
                                    <span className="font-bold text-gray-800 flex-grow text-right truncate">{data.phone}</span>
                                </div>
                                <div className="flex items-center gap-3" dir="ltr">
                                ...
                                <div className="flex items-center gap-3" dir="ltr">
                                    ...
                                    <span className="font-bold text-gray-800 flex-grow text-right truncate">{data.social}</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {exportedImage && (
                <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
                    <div className="bg-white p-6 rounded-3xl max-w-4xl w-full max-h-[90vh] flex flex-col">
                        <div className="flex justify-between items-center mb-4 border-b pb-3">
                            <h3 className="text-xl font-bold text-gray-800">تم إنشاء الصورة بنجاح!</h3>
                            <button 
                        ...
                        <div className="flex-grow overflow-auto border border-gray-200 rounded-2xl bg-gray-50 flex items-center justify-center p-4">
                            <img src={exportedImage} alt="Exported Property Card" className="shadow-lg rounded-xl max-w-full h-auto" />
                        </div>
                        ...
                        <div className="mt-6 flex justify-center gap-4">
                            <a 
                                ...
                                className="bg-zinc-900 hover:bg-black text-white font-bold py-3 px-8 rounded-2xl shadow-md transition flex items-center gap-2"
                            >
