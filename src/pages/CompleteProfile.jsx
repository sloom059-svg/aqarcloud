...
import { Loader2, Building2, Phone, Upload, Hotel, Tent, Leaf, Trees,
  ...
  CheckCircle2, Armchair, Youtube, LayoutDashboard, FileText } from "lucide-react";
...
const CITIES = Object.values(REGIONS_DATA).flat();

const ROLES = [
  { id: 'وسيط',    label: 'وسيط عقاري',   Icon: Building2, desc: 'إدارة وعرض العقارات للبيع والإيجار' },
  { id: 'شاليه',   label: 'مالك شاليه',    Icon: Waves,     desc: 'إدارة الشاليه وحجوزاته' },
  ...
  { id: 'استراحة', label: 'مالك استراحة',  Icon: Armchair,  desc: 'إدارة الاستراحة وحجوزاتها' },
];

const ALL_FEATURES = [
  { id: 'pool', name: 'مسبح', icon: Waves }, { id: 'outdoor', name: 'جلسات خارجية', icon: Trees },
  { id: 'wifi', name: 'واي فاي', icon: Wifi }, { id: 'kitchen', name: 'مطبخ', icon: UtensilsCrossed },
  { id: 'kids', name: 'ألعاب أطفال', icon: Star }, { id: 'grill', name: 'شواء', icon: Flame },
  { id: 'men', name: 'قسم رجال', icon: ShieldCheck }, { id: 'women', name: 'قسم نساء', icon: Star },
  { id: 'bed', name: 'غرف نوم', icon: Home }, { id: 'ac', name: 'مكيف', icon: Wind }
];
...
const Flame = (p) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...p}><path d="M8.5 14.5A2.5 2.5 0 0 0 11 12c0-1.38-.5-2-1-3-1.072-2.143-.224-4.054 2-6 .5 2.5 2 4.9 4 6.5 2 1.6 3 3.5 3 5.5a7 7 0 1 1-14 0c0-1.153.433-2.294 1-3a2.5 2.5 0 0 0 2.5 2.5z"/></svg>;
...
export default function CompleteProfile() {
  ...
  // إجمالي الخطوات يعتمد على نوع الحساب
  const TOTAL_STEPS = role === 'وسيط' ? 2 : 8; 
  ...
  const [brokerForm, setBrokerForm] = useState({ 
    ...
    city: "", 
    office_logo_url: "",
    license_number: "" // رقم رخصة فال/موثوق
  });

  // بيانات الشاليه/المنشأة
  const [form, setForm] = useState({
    name: '', venue_type: '', description: '', city: '', maps_url: '', images: [], youtube_urls: [],
    custom_features: [], social: { instagram: '', snapchat: '', tiktok: '', x: '' },
    page_theme: 'classic', theme_color: '#c9a96e', price_weekday: '', price_weekend: '',
    whatsapp: '', check_in_time: '14:00', check_out_time: '12:00', booking_terms: '', features: [], slug: '',
  });
  ...
  const handleRoleSelect = (selectedRole) => {
    ...
    if (selectedRole !== 'وسيط') {
      setForm(p => ({ ...p, venue_type: selectedRole }));
    }
  ...
  const handleBrokerLogoUpload = async (e) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 1. مسار الوسيط العقاري
      if (role === 'وسيط') {
        if (!brokerForm.office_name || !brokerForm.phone || !brokerForm.city) {
          setSaving(false);
          return;
        }
        ...
        // عرض شاشة النجاح الخاصة بالوسيط
        setSuccessData({ type: 'broker', name: brokerForm.office_name });
        return;
      }

      // 2. مسار المنشأة (شاليه، مخيم الخ)
      ...
      // عرض شاشة النجاح الخاصة بالمنشأة
      setSuccessData({ type: 'venue', name: form.name, url: `${window.location.origin}/place/${slug}` });
    } catch (err) {
      console.error(err);
    } finally {
      setSaving(false);
  ...
  // ══════════════════════════════════════════════════
  // شاشة النجاح الموحدة الذكية
  // ══════════════════════════════════════════════════
  if (successData) {
    const isBroker = successData.type === 'broker';
      ...
      <div dir="rtl" className="fixed inset-0 z-50 bg-[#f4f7fb] flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-500">
        ...
        {isBroker ? (
          <p className="text-slate-500 text-lg mb-10 max-w-[320px] leading-relaxed mx-auto">
            مبروك! تم إعداد ملفك <span className="font-bold text-[#15317E]">{successData.name}</span> بنجاح. بإمكانك الآن إضافة عقاراتك مباشرة!
          </p>
        ) : (
          <p className="text-slate-500 text-lg mb-10 max-w-[320px] leading-relaxed mx-auto">
            تم تجهيز صفحة مكانك بأناقة تامة. وتقدر الحين تستقبل حجوزاتك وترسل الرابط لعملائك.
          </p>
        )}

        <div className="w-full space-y-4 max-w-sm mx-auto">
          {/* زر لوحة التحكم (يختلف مساره حسب نوع الحساب) */}
          <button onClick={() => window.location.href = isBroker ? '/' : '/venue'} className="w-full py-4 bg-[#15317E] hover:bg-[#0d1e4c] text-white rounded-2xl font-bold text-lg shadow-xl shadow-[#15317E]/20 transition-all flex items-center justify-center gap-2">
            <LayoutDashboard className="w-5 h-5" /> انطلق للوحة التحكم
          </button>
          
          {/* أزرار مخصصة للوسيط */}
          {isBroker && (
             <button onClick={() => window.location.href = '/add-property'} className="w-full py-4 bg-emerald-500 hover:bg-emerald-600 text-white rounded-2xl font-bold transition-all flex items-center justify-center gap-2 shadow-sm">
               <Plus className="w-5 h-5" /> إضافة عقار جديد
             </button>
          )}

          {/* أزرار مخصصة للشاليه / المخيم */}
          {!isBroker && (
            <div className="flex gap-4">
              <button onClick={() => {
                if (navigator.share) navigator.share({ title: successData.name, url: successData.url }).catch(()=>{});
                else navigator.clipboard.writeText(successData.url); 
              ...
              <button onClick={() => window.open(successData.url, '_blank')} className="flex-1 py-4 bg-white border border-slate-200 hover:border-[#15317E] text-slate-700 hover:text-[#15317E] rounded-2xl font-bold transition-all flex items-center justify-center gap-2 shadow-sm">
  ...
  return (
    <div dir="rtl" className="min-h-screen flex flex-col items-center justify-center py-10 px-4 relative bg-[#f4f7fb] font-sans">
      ...
      <div className="w-full max-w-xl mx-auto flex flex-col h-full min-h-[600px]">
        {/* Header / Progress */}
        <div className="mb-6 px-2 flex justify-between items-center transition-opacity duration-300">
          ...
          <button onClick={() => window.location.href = '/login'} className="w-10 h-10 rounded-full bg-white shadow-sm flex items-center justify-center text-slate-400 hover:text-red-500 transition-all">
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          <div className="steps-wrapper flex-1 flex flex-col">
            ...
            <div className={`step-panel flex flex-col h-full ${getStepClass(1)}`}>
              <div className="mb-6 text-center mt-2">
                ...
                <h2 className="text-2xl md:text-3xl font-bold text-slate-800">وش نوع حسابك؟</h2>
                <p className="text-slate-500 mt-2 text-sm">حدد صفتك عشان نجهز لك الواجهة المناسبة.</p>
              </div>
              <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-3 pb-4">
                {ROLES.map(r => (
                  <button key={r.id} onClick={() => handleRoleSelect(r.id)}
                    className={`w-full relative p-5 rounded-[1.5rem] border-2 text-right transition-all flex items-center gap-4 ${role === r.id ? 'border-[#15317E] bg-blue-50/30' : 'border-slate-100 bg-white hover:border-[#15317E]/40'}`}>
                    <div className={`p-3 rounded-2xl ${role === r.id ? 'bg-[#15317E] text-white' : 'bg-slate-50 text-slate-500'}`}><r.Icon className="w-6 h-6" /></div>
            ...
            {/* ── مسار الوسيط (وسيط عقاري) - الخطوة 2 والأخيرة ── */}
            {role === 'وسيط' && (
              <div className={`step-panel flex flex-col h-full ${getStepClass(2)}`}>
                <div className="mb-6 mt-4">
                  <h2 className="text-2xl md:text-3xl font-bold text-slate-800">أهلاً بالوسيط! 👋</h2>
                  <p className="text-slate-500 mt-2 text-sm">أكمل بيانات مكتبك للبدء في نشر العقارات.</p>
                </div>
                <div className="space-y-6 flex-1 overflow-y-auto custom-scrollbar pr-2 pb-4">
                  <div>
                    <input type="text" value={brokerForm.office_name} onChange={e=>setBrokerForm(p=>({...p,office_name:e.target.value}))} placeholder="اسم المكتب العقاري" className="w-full input-clean text-[#15317E]" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-slate-500 mb-2">رقم الجوال (واتساب)</label>
                    <input type="tel" dir="ltr" value={brokerForm.phone} onChange={e=>setBrokerForm(p=>({...p,phone:e.target.value}))} placeholder="05XXXXXXXX" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 outline-none focus:border-[#15317E] text-lg font-bold tracking-widest text-left" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-slate-500 mb-2">رقم رخصة فال / موثوق (اختياري)</label>
                    <div className="relative">
                      <FileText className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
                      <input type="text" value={brokerForm.license_number} onChange={e=>setBrokerForm(p=>({...p,license_number:e.target.value}))} placeholder="أدخل رقم الرخصة" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 pr-10 outline-none focus:border-[#15317E] text-sm text-left font-mono" dir="ltr" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-slate-500 mb-3">في أي مدينة؟</label>
                    <div className="flex flex-wrap gap-2">
                      {CITIES.slice(0, 15).map(c => (
                        <button key={c} onClick={() => setBrokerForm(p => ({ ...p, city: c }))}
                          className={`px-4 py-2 rounded-full border text-sm font-medium transition-all ${brokerForm.city === c ? 'bg-[#15317E] text-white border-[#15317E]' : 'bg-transparent text-slate-600 border-slate-200 hover:border-[#15317E]'}`}>
                          {c}
                        </button>
                      ))}
                    </div>
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-500 mb-2">الشعار أو الصورة الشخصية (اختياري)</label>
                    <label className="flex items-center gap-3 h-14 px-4 border-2 border-dashed border-slate-200 rounded-2xl cursor-pointer hover:border-[#15317E]/50 transition">
                      {uploading ? <Loader2 className="w-5 h-5 animate-spin text-[#15317E]" /> : brokerForm.office_logo_url ? <img src={brokerForm.office_logo_url} className="h-8 w-8 object-cover rounded" /> : <Upload className="w-5 h-5 text-slate-400" />}
                      <span className="text-sm font-bold text-slate-600">{brokerForm.office_logo_url ? "تم رفع الشعار ✓" : "اضغط لرفع الشعار"}</span>
                      <input type="file" accept="image/*" className="hidden" onChange={handleBrokerLogoUpload} disabled={uploading} />
                    </label>
                  </div>
                </div>
              </div>
            ...
            {/* ── مسار المنشأة (شاليه/مخيم) - الخطوة 2 إلى 8 ── */}
            {role !== 'وسيط' && (
              <>
                {/* STEP 2: Name & City */}
                <div className={`step-panel flex flex-col h-full ${getStepClass(2)}`}>
                  <div className="mb-6 mt-4">
                    ...
                    <h2 className="text-2xl md:text-3xl font-bold text-slate-800">وش اسم {role} حقك؟</h2>
                  </div>
                  <div className="space-y-6 flex-1 overflow-y-auto custom-scrollbar pr-2">
                    <div>
                      <input type="text" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder={`مثال: ${role} الريم الفاخر`} className="w-full input-clean text-[#15317E]" />
                    </div>
                    <div>
                ...
                <div className={`step-panel flex flex-col h-full ${getStepClass(5)}`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                    <button onClick={() => {setForm(p => ({ ...p, page_theme: 'classic' }));}} className={`choice-card relative bg-white border-2 rounded-[2rem] p-6 text-right overflow-hidden group ${form.page_theme === 'classic' ? 'border-[#15317E] ring-2 ring-[#15317E]/20' : 'border-slate-100'}`}>
                    ...
                    <button onClick={() => {setForm(p => ({ ...p, page_theme: 'royal' })); setTimeout(() => nextStep(), 350);}} className={`choice-card relative bg-slate-900 border-2 rounded-[2rem] p-6 text-right overflow-hidden group hover:border-[#c9a96e] ${form.page_theme === 'royal' ? 'border-[#c9a96e]' : 'border-slate-800'}`}>
                  ...
                  <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                    <div className="flex flex-wrap gap-2.5">
                      {ALL_FEATURES.map(f => {
                        ...
                        return (
                          <button key={f.id} onClick={() => setForm(p => ({ ...p, features: isSelected ? p.features.filter(x => x !== f.name) : [...p.features, f.name] }))} 
                            className={`px-5 py-3 rounded-full border font-bold text-sm flex items-center gap-2 transition-all ${isSelected ? 'bg-[#15317E] text-white border-[#15317E] shadow-md scale-105' : 'bg-white text-slate-600 border-slate-200'}`}>
          ...
          <div className="mt-6 pt-4 flex justify-between items-center bg-white z-10 border-t border-slate-50">
            ...
            {currentStep === TOTAL_STEPS && (
              <button onClick={handleSubmit} disabled={saving} className="px-8 py-3 rounded-full font-bold text-white bg-emerald-500 hover:bg-emerald-600 shadow-lg shadow-emerald-500/30 transition-all flex items-center gap-2 active:scale-95">
                {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Check className="w-4 h-4" /> <span>{role === 'وسيط' ? 'إكمال التسجيل' : 'احفظ وانشر الشاليه'}</span></>}
              </button>
            )}
          </div>
        </div>
