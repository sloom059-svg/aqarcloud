...
import {
  ...
  Sparkles, Tv, Dumbbell, Bath, Wind, Sun, Music, Camera, Heart, Gift, Crown, Mountain
} from 'lucide-react';
  ...
  const BookingDialog = ({ dark }) => {
    ...
    <Dialog open={bookingOpen} onOpenChange={setBookingOpen}>
      <DialogTrigger asChild>
        {dark ? (
          <button className="text-[#020617] px-8 py-3.5 rounded-2xl font-black text-sm transition-all active:scale-95 flex items-center gap-2"
          ...
          <button className="text-white px-8 py-3.5 rounded-xl font-bold text-sm transition-all shadow-md active:scale-95"
      ...
      <DialogContent dir="rtl"
        className="max-w-sm w-[95vw] rounded-[2rem] shadow-2xl p-0 overflow-hidden border-0 overflow-y-auto max-h-[92vh]"
        style={{ fontFamily: "'Tajawal', sans-serif" }}>

        {/* رأس النافذة — بلون الثيم */}
        <div className="px-6 pt-6 pb-5 relative" style={{ background: `linear-gradient(135deg, ${accent}18, ${accent}08)`, borderBottom: `2px solid ${accent}20` }}>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-base font-black text-gray-900 leading-tight">{venue.name}</h2>
                <p className="text-xs font-bold" style={{ color: accent }}>طلب حجز إقامة</p>
              </div>
            </div>
            <div className="text-left border-r-4 pr-3" style={{ borderColor: accent }}>
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Booking Request</p>
              <p className="text-xs font-black text-gray-600">{new Date().toLocaleDateString('ar-SA', { day:'numeric', month:'long', year:'numeric' })}</p>
            </div>
          </div>
          {/* خط فاصل مزخرف */}
          <div className="w-full h-px" style={{ background: `linear-gradient(to left, transparent, ${accent}40, transparent)` }} />
        </div>

        {bookingDone ? (
          <div className="text-center py-10 px-6">
            <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: `${accent}15`, color: accent }}>
              <CheckCircle2 className="w-9 h-9" />
            </div>
            <h3 className="text-xl font-black text-gray-900 mb-1">تم إرسال الطلب!</h3>
            <p className="text-sm text-gray-400 font-bold mb-6">سيتواصل معك صاحب الشاليه قريباً</p>
            <button onClick={() => { setBookingOpen(false); setBookingDone(false); }}
              className="w-full h-12 rounded-xl font-black text-white text-sm shadow-md"
              style={{ background: accent }}>
              حسناً، شكراً
            </button>
          </div>
          ...
          <div className="px-6 py-5 space-y-4">
            ...
            {bookingForm.check_in && bookingForm.check_out && (() => {
              ...
              let cur = new Date(bookingForm.check_in);
            ...
            {/* حقل الاسم */}
            <div>
              <label className="block text-xs font-black text-gray-500 mb-1.5">الاسم الكريم <span className="text-red-400">*</span></label>
              <div className="flex items-center h-12 rounded-xl border-2 px-3 gap-2 bg-gray-50 transition-all focus-within:bg-white" style={{ '--tw-border-color': '#e5e7eb' }}
                onFocus={e => e.currentTarget.style.borderColor = accent}
                onBlur={e => e.currentTarget.style.borderColor = '#e5e7eb'}>
                <input className="flex-1 bg-transparent border-none outline-none text-sm font-bold text-gray-800 placeholder:text-gray-300"
                  value={bookingForm.client_name} onChange={e => setBookingForm(p => ({ ...p, client_name: e.target.value }))} placeholder="الاسم الثلاثي" />
              </div>
            </div>

            {/* حقل الجوال */}
            <div>
              <label className="block text-xs font-black text-gray-500 mb-1.5">رقم الجوال <span className="text-red-400">*</span></label>
              <div className="flex items-center h-12 rounded-xl border-2 px-3 gap-2 bg-gray-50 transition-all focus-within:bg-white" style={{ borderColor: '#e5e7eb' }}
                onFocus={e => e.currentTarget.style.borderColor = accent}
                onBlur={e => e.currentTarget.style.borderColor = '#e5e7eb'}>
                <input className="flex-1 bg-transparent border-none outline-none text-sm font-bold text-gray-800 placeholder:text-gray-300" dir="ltr"
                  value={bookingForm.client_phone} onChange={e => setBookingForm(p => ({ ...p, client_phone: e.target.value }))} placeholder="05xxxxxxxx" />
              </div>
            </div>

            {/* فترة الإقامة */}
            <div>
              <label className="block text-xs font-black text-gray-500 mb-1.5">فترة الإقامة <span className="text-red-400">*</span></label>
              {(bookingForm.check_in && bookingForm.check_out) ? (
                <div className="flex items-center justify-between rounded-xl px-4 py-3 border-2 bg-gray-50" style={{ borderColor: `${accent}50` }}>
                  ...
                  <button onClick={() => setBookingForm(p => ({ ...p, check_in: '', check_out: '' }))}
                    className="text-xs font-bold text-red-400 hover:text-red-600">تغيير</button>
                </div>
              ) : (
            ...
            {/* ملاحظات */}
            <div>
              <label className="block text-xs font-black text-gray-500 mb-1.5">ملاحظات <span className="text-gray-300 font-bold">(اختياري)</span></label>
              <textarea className="w-full rounded-xl border-2 bg-gray-50 px-3 py-2.5 text-sm font-semibold text-gray-800 placeholder:text-gray-300 outline-none resize-none transition-all focus:bg-white"
                style={{ borderColor: '#e5e7eb' }}
                onFocus={e => e.target.style.borderColor = accent}
                onBlur={e => e.target.style.borderColor = '#e5e7eb'}
                value={bookingForm.notes} onChange={e => setBookingForm(p => ({ ...p, notes: e.target.value }))}
                placeholder="أي طلبات خاصة..." rows={2} />
            </div>

            {/* زر الإرسال */}
            <button onClick={handleBook}
              className="w-full h-13 py-3.5 text-sm font-black text-white rounded-xl shadow-lg transition-all active:scale-95 disabled:opacity-50"
              style={{ background: accent }}
            ...
            <div className="flex items-center justify-center gap-1.5 pt-1">
              ...
              <span className="text-[10px] font-bold text-gray-300 tracking-widest uppercase">Powered by Aqar Cloud</span>
              <div className="w-1.5 h-1.5 rounded-full" style={{ background: accent }} />
            </div>
