  ...
  const BookingDialog = ({ dark }) => (
    <Dialog open={bookingOpen} onOpenChange={setBookingOpen}>
      ...
      <DialogContent dir="rtl"
        className={`max-w-md w-[95vw] rounded-[2rem] shadow-2xl font-cairo p-6 ${dark ? 'bg-[#0f172a] border border-[#d4af37]/30' : 'bg-white border-0'}`}>
        <DialogHeader className="mb-4">
          <DialogTitle className={`text-2xl font-black ${dark ? 'text-white' : 'text-gray-900'}`}>طلب حجز · {venue.name}</DialogTitle>
          {bookingForm.check_in && bookingForm.check_out && (() => {
            ...
            <div className="bg-red-50 text-red-600 p-3 rounded-xl text-xs font-bold mt-3 border border-red-100 flex items-start gap-2">
        ...
        {bookingDone ? (
          <div className="text-center py-8">
            ...
            <h3 className={`text-2xl font-black mb-2 ${dark ? 'text-white' : 'text-gray-900'}`}>تم إرسال الطلب بنجاح!</h3>
            <p className={`font-medium mb-8 ${dark ? 'text-slate-400' : 'text-gray-500'}`}>سيتم التواصل معك قريباً لتأكيد الحجز.</p>
            <Button onClick={() => { setBookingOpen(false); setBookingDone(false); }}
              className="w-full h-14 rounded-xl font-bold text-base text-white" style={{ background: accent }}>
              حسناً، شكراً
            </Button>
          </div>
        ) : (
          <div className="space-y-4">
            <div className="space-y-1.5">
              <Label className={`font-bold text-xs ${dark ? 'text-slate-300' : 'text-gray-700'}`}>الاسم الكريم <span style={{ color: dark ? ROYAL_GOLD : '#ef4444' }}>*</span></Label>
              <Input className={`h-12 rounded-xl font-semibold ${dark ? 'border-[#1e293b] bg-[#020617] text-white placeholder:text-slate-600' : 'border-gray-200 bg-gray-50'}`}
                value={bookingForm.client_name} onChange={e => setBookingForm(p => ({ ...p, client_name: e.target.value }))} placeholder="الاسم الثلاثي" />
            </div>
            <div className="space-y-1.5">
              <Label className={`font-bold text-xs ${dark ? 'text-slate-300' : 'text-gray-700'}`}>رقم الجوال <span style={{ color: dark ? ROYAL_GOLD : '#ef4444' }}>*</span></Label>
              <Input className={`h-12 rounded-xl font-semibold ${dark ? 'border-[#1e293b] bg-[#020617] text-white placeholder:text-slate-600' : 'border-gray-200 bg-gray-50'}`}
                value={bookingForm.client_phone} onChange={e => setBookingForm(p => ({ ...p, client_phone: e.target.value }))} placeholder="05xxxxxxxx" dir="ltr" />
            </div>
            <div className="space-y-1.5">
              <Label className={`font-bold text-xs ${dark ? 'text-slate-300' : 'text-gray-700'}`}>اختر فترة الإقامة <span style={{ color: dark ? ROYAL_GOLD : '#ef4444' }}>*</span></Label>
              {(bookingForm.check_in && bookingForm.check_out) ? (
                <div className={`flex items-center justify-between rounded-xl px-4 py-3 border ${dark ? 'bg-[#020617] border-[#1e293b]' : 'bg-gray-50 border-gray-200'}`}>
                  <span className={`text-sm font-bold ${dark ? 'text-white' : 'text-gray-800'}`}>{bookingForm.check_in} → {bookingForm.check_out}</span>
                  <button onClick={() => setBookingForm(p => ({ ...p, check_in: '', check_out: '' }))}
                    className="text-xs text-red-400 font-bold hover:text-red-600">تغيير</button>
                </div>
              ) : (
                <div className={`border rounded-xl p-3 ${dark ? 'border-[#1e293b] bg-[#020617]' : 'border-gray-100 bg-gray-50/50'}`}>
                  <VenueCalendar
            ...
            <div className="space-y-1.5">
              <Label className={`font-bold text-xs ${dark ? 'text-slate-300' : 'text-gray-700'}`}>ملاحظات (اختياري)</Label>
              <Textarea className={`rounded-xl font-semibold resize-none ${dark ? 'border-[#1e293b] bg-[#020617] text-white placeholder:text-slate-600' : 'border-gray-200 bg-gray-50'}`}
                value={bookingForm.notes} onChange={e => setBookingForm(p => ({ ...p, notes: e.target.value }))} placeholder="أي طلبات خاصة..." rows={2} />
            </div>
            <Button onClick={handleBook}
              className="w-full h-14 text-base font-black text-white rounded-xl shadow-lg mt-2"
              style={{ background: accent }}
              ...
              {bookMutation.isPending ? <><Loader2 className="w-5 h-5 ml-2 animate-spin" />جاري الإرسال...</> : 'تأكيد إرسال الطلب'}
            </Button>
          </div>
        )}
      </DialogContent>
