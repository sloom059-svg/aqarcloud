...
export default function VenueWizard() {
  ...
  if (successVenue) {
    ...
    return (
      <div dir="rtl" className={`min-h-screen flex flex-col items-center justify-center p-6 text-center relative overflow-hidden ${isRoyal ? 'bg-[#0a0e1a]' : 'bg-[#F7F7F7]'}`}>
        ...
        <div className="relative z-10 max-w-sm w-full">
          <div className="mb-8" style={{ animation: 'pop 0.6s cubic-bezier(0.16,1,0.3,1)' }}>
            <div className={`w-28 h-28 rounded-full mx-auto flex items-center justify-center shadow-2xl ${isRoyal ? 'bg-[#d4af37]' : 'bg-white'}`}>
              <PartyPopper className={`w-14 h-14 ${isRoyal ? 'text-[#0a0e1a]' : 'text-[#FF385C]'}`} />
            </div>
          </div>

          <div style={{ animation: 'fadeUp 0.6s ease-out 0.2s both' }}>
            <h1 className={`text-3xl font-black mb-3 ${isRoyal ? 'text-[#d4af37]' : 'text-zinc-950'}`}>🎉 مبروك!</h1>
            <p className={`${isRoyal ? 'text-white/80' : 'text-zinc-600'} text-base mb-2 leading-relaxed`}>
              تم نشر صفحة <span className="font-bold">{successVenue.name}</span> بنجاح
            </p>
            <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold mb-8 ${isRoyal ? 'bg-[#d4af37]/15 text-[#d4af37] border border-[#d4af37]/30' : 'bg-[#FF385C]/10 text-[#FF385C] border border-[#FF385C]/15'}`}>
          ...
          <div className="space-y-3" style={{ animation: 'fadeUp 0.6s ease-out 0.4s both' }}>
            <button onClick={() => window.open(successVenue.url, '_blank')}
              className={`w-full py-4 rounded-2xl font-bold text-base shadow-xl transition-all flex items-center justify-center gap-2 active:scale-95 ${isRoyal ? 'bg-[#d4af37] text-[#0a0e1a] hover:bg-[#c9a227]' : 'bg-[#FF385C] text-white hover:bg-[#E31C5F]'}`}>
              <Eye className="w-5 h-5" /> عرض صفحتي
            </button>
            <button onClick={() => navigate('/venue')}
              className={`${isRoyal ? 'bg-white/10 text-white border-white/20 hover:bg-white/20' : 'bg-white text-zinc-900 border-zinc-200 hover:bg-zinc-50'} w-full py-4 rounded-2xl font-bold text-base border transition-all flex items-center justify-center gap-2 active:scale-95`}>
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#F7F7F7] font-sans flex flex-col items-center py-6 sm:py-8 px-4 text-zinc-950"
      style={{ backgroundImage: 'radial-gradient(at 0% 0%, rgba(255,56,92,0.08) 0px, transparent 46%), radial-gradient(at 100% 0%, rgba(34,34,34,0.05) 0px, transparent 48%)', backgroundAttachment: 'fixed' }}>
      <style dangerouslySetInnerHTML={{__html: `
        @import url('https://fonts.googleapis.com/css2?family=IBM+Plex+Sans+Arabic:wght@300;400;500;600;700&display=swap');
      ...
      <div className="w-full max-w-xl mx-auto">
        ...
        <div className="bg-white rounded-[2rem] border border-zinc-200 shadow-[0_22px_55px_-28px_rgba(34,34,34,0.35)] p-6 md:p-10 min-h-[460px] flex flex-col">
          <div className="flex-1">

            {/* خطوة ١: نوع المكان */}
            {step === 1 && (
              <div>
                <h2 className="text-2xl font-bold text-zinc-950 mb-1">ما نوع منشأتك؟</h2>
                <p className="text-zinc-500 text-sm mb-8">اختر النوع المناسب لبدء الإعداد</p>
                <div className="grid grid-cols-2 gap-4">
                  {VENUE_TYPES.map(t => (
                    <button key={t.id} onClick={() => { set('venue_type', t.id); setTimeout(next, 250); }}
                      className={`p-6 rounded-2xl border-2 flex flex-col items-center gap-3 transition-all active:scale-95 ${form.venue_type === t.id ? 'border-[#FF385C] bg-[#FF385C]/5' : 'border-zinc-100 hover:border-zinc-200'}`}>
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${form.venue_type === t.id ? 'bg-[#FF385C] text-white' : 'bg-zinc-50 text-zinc-500'}`}>
                        <t.Icon className="w-7 h-7" />
                      </div>
                      <span className="font-bold text-zinc-800">{t.label}</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            {step === 2 && (
              <div>
                <h2 className="text-2xl font-bold text-zinc-950 mb-1">المعلومات الأساسية</h2>
                <p className="text-zinc-500 text-sm mb-8">اسم المنشأة والمدينة</p>
                <div className="space-y-5">
                  <div>
                    <label className="block text-sm font-bold text-zinc-700 mb-2">اسم المنشأة *</label>
                  ...
                  <div>
                    ...
                    <textarea value={form.description} onChange={e => set('description', e.target.value)} rows={3} placeholder="نبذة جميلة عن المنشأة..."
                      className="w-full px-4 py-3.5 bg-zinc-50 border border-zinc-200 rounded-2xl focus:border-[#FF385C] focus:ring-2 focus:ring-[#FF385C]/10 outline-none transition-all text-sm font-medium resize-none" />
                  </div>
                </div>
              </div>
            )}
            ...
            {step === 3 && (
              <div>
                <h2 className="text-2xl font-bold text-zinc-950 mb-1">المميزات والخدمات</h2>
                <p className="text-zinc-500 text-sm mb-8">اختر ما يتوفر في منشأتك</p>
                <div className="flex flex-wrap gap-2.5">
                  {ALL_FEATURES.map(f => (
                    <button key={f} onClick={() => toggleFeature(f)}
                      className={`px-4 py-2.5 rounded-full text-sm font-bold border transition-all active:scale-95 ${form.features.includes(f) ? 'bg-[#FF385C] text-white border-[#FF385C]' : 'bg-white text-zinc-700 border-zinc-200 hover:border-zinc-300'}`}>
                      {f}
                    </button>
                  ))}
                </div>
            ...
            {step === 4 && (
              <div>
                <h2 className="text-2xl font-bold text-zinc-950 mb-1">الأسعار</h2>
                <p className="text-zinc-500 text-sm mb-8">سعر الليلة (يمكنك تركها فارغة)</p>
                <div className="space-y-5">
                  <div>
                    <label className="block text-sm font-bold text-zinc-700 mb-2">سعر أيام الأسبوع</label>
                    <div className="relative">
                      <input type="number" value={form.price_weekday} onChange={e => set('price_weekday', e.target.value)} placeholder="0"
                        className="w-full px-4 py-3.5 bg-zinc-50 border border-zinc-200 rounded-2xl focus:border-[#FF385C] focus:ring-2 focus:ring-[#FF385C]/10 outline-none text-sm font-bold" dir="ltr" />
                      <span className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-500 text-sm font-bold">ر.س</span>
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-zinc-700 mb-2">سعر نهاية الأسبوع <span className="text-zinc-500 font-normal">(خميس/جمعة)</span></label>
                    <div className="relative">
            ...
            {step === 5 && (
              <div>
                <h2 className="text-2xl font-bold text-zinc-950 mb-1">صور المنشأة</h2>
                <p className="text-zinc-500 text-sm mb-3">أضف صوراً جذابة (حتى ١٠ صور)</p>
                ...
                <div className="grid grid-cols-3 gap-3 mb-4">
                  {form.images.map((img, i) => (
                    <div key={i} className="relative aspect-square rounded-2xl overflow-hidden group">
                      ...
                      {i === 0 && <span className="absolute top-1.5 right-1.5 bg-[#FF385C] text-white text-[9px] font-bold px-2 py-0.5 rounded-full">الغلاف</span>}
            ...
            {step === 6 && (
              <div>
                <h2 className="text-2xl font-bold text-zinc-950 mb-1">التواصل والمظهر</h2>
                <p className="text-zinc-500 text-sm mb-8">رقم الحجز وثيم الصفحة</p>
                <div className="space-y-5">
                  <div>
                    <label className="block text-sm font-bold text-zinc-700 mb-2">رقم واتساب للحجز *</label>
                    <input value={form.whatsapp} onChange={e => set('whatsapp', e.target.value)} placeholder="9665xxxxxxxx" dir="ltr"
                      className="w-full px-4 py-3.5 bg-zinc-50 border border-zinc-200 rounded-2xl focus:border-[#FF385C] focus:ring-2 focus:ring-[#FF385C]/10 outline-none text-sm font-bold text-right" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-zinc-700 mb-3">ثيم الصفحة</label>
                    <div className="grid grid-cols-2 gap-4">
                      <button onClick={() => set('page_theme', 'classic')}
                        className={`p-5 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all ${form.page_theme === 'classic' ? 'border-[#FF385C] bg-[#FF385C]/5' : 'border-zinc-100'}`}>
                      ...
                      <button onClick={() => set('page_theme', 'royal')}
                        className={`p-5 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all ${form.page_theme === 'royal' ? 'border-[#d4af37] bg-[#d4af37]/5' : 'border-zinc-100'}`}>
            ...
            {step === 7 && (
              <div>
                <h2 className="text-2xl font-bold text-zinc-950 mb-1">مراجعة أخيرة</h2>
                <p className="text-zinc-500 text-sm mb-8">تأكد من البيانات قبل النشر</p>
                <div className="space-y-3">
                  {[
                    ...
                    <div key={i} className="flex items-center gap-3 bg-zinc-50 rounded-2xl p-3.5 border border-zinc-100">
                      <div className="w-9 h-9 rounded-xl bg-white flex items-center justify-center text-[#FF385C] shadow-sm"><r.icon className="w-4 h-4" /></div>
                      <span className="text-xs text-zinc-400 font-semibold">{r.label}</span>
                      <span className="text-sm font-bold text-zinc-800 mr-auto">{r.val}</span>
                    </div>
                  ))}
                </div>
              </div>
